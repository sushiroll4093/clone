...
(Note, as of 15Sep2018, the yarn install threw errors, which were resolved by installing yarn without `1.7.0-1`, specifically `sudo apt-get update && sudo apt-get install yarn`)
    ...
    sysadmin@appserver:/var/canvas$ sudo apt-get update && sudo apt-get install yarn=1.7.0-1
